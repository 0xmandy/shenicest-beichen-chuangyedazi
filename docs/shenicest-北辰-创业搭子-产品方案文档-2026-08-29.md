...
给还没开公司、但已经有 MVP 或创业意向的个人，配一个把政策补贴和开办流程算清楚的 AI 创业管家。他把自己的情况说一次，管家去 175 条政策、44 笔补贴条款、67 个金融产品、17 类办事流程里逐条比对，告诉他三件事：现在就能报哪几笔、开完公司能解锁哪几笔、开公司这件事今天该做哪一步。
...
flowchart TB
    ...
    P["数据主轴 · 一份画像 12 字段<br/>企业阶段 · 行业 · 注册地 · 成立年限<br/>人数 · 资质 · 毕业状态 · 出海<br/>融资条件 · 城市 · 办事分支 · 风险自评"]
    ...
    BUS["动作总线<br/>待办清单 · 行政日历 · 风险清单"]

    DATA["数据底座<br/>政策库 175 条 · 补贴条款 44 笔<br/>金融库 76 条 · 办事流程 17 页"]

    EV["证据层 · 数字防火墙<br/>原文本地留档 · 逐字校验 324 个校验点<br/>官方 URL 可回链"]

    OPEN["开放层<br/>MCP server · REST API · Skill 包"]
...
| 二 · 开公司与办事效率 | 企业服务专业化 | 17 页办事流程，北京官方口径 | 开公司之前三节点（选城市 / 选身份 / 行业区别）、开公司分步向导、每步的材料成本时间、官方入口链接、加进待办 | 已实现 |
...
flowchart TD
    ...
    A4 -- "定了，注册在北京" --> B0["接图 B-2"]
```

**图 B-2　e 窗通一网通办，到执照下来**

```mermaid
flowchart TD
    B0["首页 猜您需要 · 开公司<br/>仅未注册身份可见"] --> B1["分步向导<br/>第一步 名称自主申报"]
    B1 --> B2a["卡片摊开：线下办理 · 成本费用<br/>预计时间 · 材料 · 参考资料"]
    B2a --> B2b["进入 e 窗通<br/>ect.scjgj.beijing.gov.cn"]
    B2b --> B3["第二步 一网通办一次填报<br/>执照 · 公章 · 涉税 · 社保 · 开户预约"]
    B3 --> B4{"一天办结"}
    B4 -- "卡住了" --> B6["问老周<br/>答前三步 + 在哪办"]
    B6 --> B3
    B4 -- "办结" --> B5["办理结果服务包<br/>邮寄或一窗领取"]
    B5 --> B7["第三步 银行开户"]
    B7 --> B8["第四步 税务报到<br/>税费种认定 → 三方协议 → 票种核定"]
    B8 --> B9["加进我的待办，不带日期<br/>时限以受理机关公告为准"]
    B9 --> C0["注册完成，接图 B-3"]
```
...
```mermaid
flowchart TD
    C0["注册完成"] --> C1["去 我的 · 企业档案<br/>把企业阶段改成已注册"]
    C1 --> C2["applyProfileChange 全线重算"]
    C2 --> D1["补贴：33 笔法人门槛解除<br/>重新判定并排序"]
    C2 --> D2["政策：阶段加分<br/>从人才支持转到融资金融"]
...
```mermaid
flowchart TD
    F0["找机会 · 金融机会"] --> F1["勾融资条件，八项多选<br/>有知识产权 / 有不动产可抵押 / 专精特新<br/>有数据资产 / 有应收账款 / 有跨境结算<br/>想引股权投资 / 要纯信用"]
    F1 --> F2["scoreFinance 打分<br/>67 个产品逐个算"]
    F2 --> F3{"分档"}
    F3 -- "低于 8 分" --> F5["进全库浏览"]
    F3 -- "8 分及以上" --> F4["强匹配短名单，接图 C-2"]
```
...
```mermaid
flowchart TD
    F4["强匹配短名单"] --> G1["产品卡四要素<br/>额度 · 期限 · 利率 · 门槛"]
    G1 --> G2{"原表写了具体数吗"}
    G2 -- "写了" --> G3["原样显示<br/>28 个有额度，13 个有利率口径"]
    G2 -- "写的是根据企业实际情况不定" --> G4["显示 以机构核定为准<br/>不折算不估算不换口径"]
    G3 --> H1["点开看门槛原文"]
    G4 --> H1
    H1 --> H2["叠加检查<br/>9 条金融类政策贴息可否叠加"]
    H2 --> H3["生成对接申请卡"]
    H3 --> J1["同时往待办与日历各写一条<br/>准备资料，截止三天后"]
    J1 --> I1["卡片进入六步流转，见下表"]
```

对接卡生成之后按北辰金融服务六步流转，名称来自命题 PPT，产品照搬，不另起一套自己的说法：
...
```mermaid
flowchart TD
    K0["小事广场加号"] --> K1["说一句话<br/>不填表不选分类"]
    K1 --> K2{"正则判四类"}
    K2 -- "疑问句" --> L1["发到广场公开卡"]
    L1 --> L1a["政策类问题<br/>挂政策库原文"]
    K2 -- "完成态" --> L2["勾掉对应待办"]
    L2 --> L2a["日历标完成 · 解除风险<br/>生成下一步 · 进度加分"]
    K2 -- "感谢" --> L3["广场感谢卡"]
    L3 --> L3a["对方口碑 +1<br/>触发积分"]
    K2 -- "陈述句" --> L4["写进看板<br/>阶段 · 进度 · 待办 · 风险"]
    L4 --> L4a["进行政日历<br/>广场私密卡"]
    L1a --> M1["全部落进 我的小事"]
    L2a --> M1
    L3a --> M1
    L4a --> M1
...
**免费的部分与付费的部分怎么分。** 免费层是信任的入口，也是园区漏斗要的东西，不设闸：175 条政策匹配、44 笔补贴的金额与条件、够不够得着的四态判定、每一条的官方原文回链、17 类办事流程的主干步骤。用户先验证这个产品说的是不是真的，再决定要不要付钱。付费层是办事细则，也就是他真要动手那一刻缺的那一层。
...
最后一项与 5.1 的按件付费是同一件事，积分是它的免费通道。攒够分的人不用付钱，这是给广场上真在帮别人的那批人的回报。
...
种子内容已经就位：44 笔补贴条款、17 页办事流程、67 个金融产品。冷启动不需要用户生成内容也能用。
...
| 类别 | 具体内容 | 敏感度 | 存在哪 | 谁能看 |
|---|---|---|---|---|
| 公共知识 | 政策库 175 条、补贴条款 44 笔、办事流程 17 页 | 公开 | 内联在静态 HTML 里 | 所有人 |
...
- 不做融资建议与产品推荐排序的商业化，就不构成投资顾问或助贷。5.1 把主收入放在用户会员上而不是服务商对接位，排序这件事就没有一个可以被买走的入口
...
- 17 页办事流程，北京官方口径，含开公司之前的三节点（选城市 / 选身份 / 行业区别）与办经营许可证页
- 金融匹配：67 产品打分，四要素原文，9 条贴息政策，对接卡按北辰 SOP 六步
- 风险体检：按已注册 / 未注册分两套维度
- 小事引擎四路由，真写入待办与日历
- 管家老周三个知识源问答（办事流程 / 金融 / 政策），25 条测试问法逐条验过路由
- MCP server、REST API、Skill 包，与原型打分逐条对账一致
- 办经营许可证页：AI 产品开始收费该办 EDI 证还是 ICP 证，判据取自《电信业务分类目录（2015 年版）》原文并可点回，四种收费形态各给一条路径
- 开放层已部署在 `https://chuangyedazi-api.safepanda.workers.dev/`，入口在「我的」页的接到你常用的 AI；浏览器打开是落地页，AI 取同一地址拿到安装说明
- 三条自查流水线：语法检查、23 个状态的截图与横向溢出探针、45 条档案连带重算与判定口径断言
...
- 补贴办理细则（`data/policy-subsidy/…补贴办理细则与办理地点….md`）没有接进补贴详情页。全文搜不到办理地点这个字段，用户在补贴卡上看得到能拿多少与够不够格，看不到去哪办
